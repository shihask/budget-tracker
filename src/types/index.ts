...
/** Derive the union from the constant, never the other way round — every call site
 *  spells the value as MASTER_TYPES.PERSON, so a typo is a compile error rather
...
/** A person or merchant in the Master Directory. In v1.60 this is a standalone
 *  directory — nothing references it. `transactions.master_id` lands next release. */
export interface Master {
  id: string
  user_id?: string
  name: string
  /** Generated by PostgreSQL (`trim(name)`). Never write this field — an insert or
   *  patch containing it errors. Required, not optional: `select('*')` always
   *  returns it, and `MasterForm` in useSupabaseData removes it from the write path. */
  display_name: string
  type: MasterType
  /** Reserved for a future release — no UI writes this in v1.60. */
  category_id: string | null
  /** People only; merchants leave this null. Stored exactly as typed. */
  phone: string | null
  /** Reserved — avatars render initials in v1.60. */
  photo_url: string | null
  notes: string | null
  created_at?: string
  updated_at?: string
  /** Soft delete. Rows held in AppState are always live, so this is never
   *  populated in memory — the load query filters on it. */
